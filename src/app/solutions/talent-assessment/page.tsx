"use client"
import { Button } from "@/app/components/ui/button";
import Navbar from "@/app/components/layout/Navbar";
import Footer from "@/app/components/layout/Footer";
import Link from "next/link";

import { ArrowLeft, ClipboardCheck, ArrowRight, Check, Star, Download } from "lucide-react";

const TalentAssessment = () => (
  <div className="min-h-screen flex flex-col">
    <Navbar />

    {/* Hero Section */}
    <section className="pt-16 pb-12 md:pt-24 md:pb-16 relative overflow-hidden bg-gradient-to-br from-green-50 to-white">
      {/* Background decorations */}
      <div className="blur-circle blur-circle-primary w-[400px] h-[400px] -top-64 -right-64 animate-pulse-soft"></div>
      <div className="blur-circle blur-circle-secondary w-[300px] h-[300px] bottom-40 left-0 animate-pulse-soft"></div>

      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div className="animate-fade-in">
            <div className="inline-block bg-gradient-to-r from-green-100 to-green-200 px-4 py-2 rounded-full mb-4">
              <span className="text-green-700 font-medium">Skills Verification Platform</span>
            </div>

            <h1 className="text-[39px] md:text-[47px] font-bold mb-6 leading-tight">
              <span className="text-green-600">Accurate</span>, Fair, and Data-Driven Skill Evaluation
            </h1>

            <p className="text-[17px] md:text-[19px] text-gray-600 mb-8">
              Empower your team to assess skills and potential with AI-powered, customizable, role-based assessments covering both technical and soft skills.
            </p>

            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="https://portal.disamina.ai/signin" rel="noreferrer noopener" target="_blank">
                <Button className="bg-green-600 hover:bg-green-700 text-white text-[17px] py-6 px-8 rounded-md flex gap-2 items-center transform transition-transform hover:scale-105">
                  Explore Assessments
                  <ArrowRight className="w-5 h-5" />
                </Button>
              </Link>

            </div>
          </div>

          <div className="flex justify-center animate-fade-in">
            <img
              src="/images/AI Assessment_home.webp"
              alt="Talent Assessment"
              className="rounded-xl shadow-lg object-cover max-h-[450px] border-4 border-white"
              loading="lazy"
            />
          </div>
        </div>
      </div>
    </section>

    {/* How It Works Section */}
    <section className="py-16 md:py-24 bg-white">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">How It Works</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Four simple steps to implement comprehensive skills assessment into your hiring workflow
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
          {[
            {
              step: "01",
              title: "Select Assessment",
              description: "Choose from 100+ pre-built templates or create a custom assessment",
              icon: "📋"
            },
            {
              step: "02",
              title: "Invite Candidates",
              description: "Send automated assessment invitations to your candidates",
              icon: "✉️"
            },
            {
              step: "03",
              title: "AI Evaluation",
              description: "Our AI scores responses and provides detailed skill analysis",
              icon: "🤖"
            },
            {
              step: "04",
              title: "Review Results",
              description: "Get comprehensive reports with skill gaps and benchmarking",
              icon: "📊"
            }
          ].map((item, index) => (
            <div key={index} className="bg-white rounded-xl border border-gray-100 p-8 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
              <div className="absolute top-0 right-0 bg-green-50 text-green-600 px-3 py-1 text-sm font-bold rounded-bl-lg">
                Step {item.step}
              </div>
              <div className="h-16 w-16 bg-green-100 rounded-full flex items-center justify-center text-2xl mb-6">
                {item.icon}
              </div>
              <h3 className="text-xl font-bold mb-3">{item.title}</h3>
              <p className="text-gray-600">{item.description}</p>

              {index < 3 && (
                <div className="hidden md:block absolute -right-4 top-1/2 transform -translate-y-1/2 text-green-300">
                  <ArrowRight size={24} />
                </div>
              )}
            </div>
          ))}
        </div>

        <div className="mt-12 text-center">
          <Link href="https://portal.disamina.ai/signin" rel="noreferrer noopener" target="_blank">
            <Button className="bg-green-600 hover:bg-green-700 text-white">
              Create Your First Assessment
            </Button>
          </Link>
        </div>
      </div>
    </section>

    {/* Benefits Section */}
    <section className="py-16 md:py-24 bg-gray-50">
      <div className="container mx-auto px-4 md:px-6">
        <div className="text-center mb-16">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Benefits of Our Talent Assessments</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Make data-driven hiring decisions with comprehensive skill evaluation tools
          </p>
        </div>

        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[
            {
              title: "100+ Validated Templates",
              description: "Role-based assessment templates with instant benchmarking against industry standards",
              icon: <Star className="w-6 h-6 text-green-600" />
            },
            {
              title: "Custom Assessment Builder",
              description: "Create your own assessments with AI recommendations for skills to test",
              icon: <Check className="w-6 h-6 text-green-600" />
            },
            {
              title: "Skill Gap Analysis",
              description: "Identify specific areas where candidates need development or training",
              icon: <Check className="w-6 h-6 text-green-600" />
            },
            {
              title: "Comprehensive Test Types",
              description: "Technical, cognitive, behavioral, and communication assessments in one platform",
              icon: <Star className="w-6 h-6 text-green-600" />
            },
            {
              title: "Bias Reduction",
              description: "Objective skill measurement helps create a more fair and inclusive hiring process",
              icon: <Check className="w-6 h-6 text-green-600" />
            },
            {
              title: "Predictive Hiring Metrics",
              description: "Use assessment data to predict job performance and cultural fit",
              icon: <Star className="w-6 h-6 text-green-600" />
            }
          ].map((benefit, index) => (
            <div key={index} className="bg-white rounded-lg p-8 shadow-sm hover:shadow-md transition-all hover:-translate-y-1 duration-300">
              <div className="mb-4 inline-block p-3 bg-green-50 rounded-lg">
                {benefit.icon}
              </div>
              <h3 className="text-xl font-bold mb-3">{benefit.title}</h3>
              <p className="text-gray-600">{benefit.description}</p>
            </div>
          ))}
        </div>
      </div>
    </section>

    {/* CTA Section */}
    <section className="py-16 md:py-20 bg-gradient-to-r from-green-500 to-green-600 text-white">
      <div className="container mx-auto px-4 md:px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl md:text-4xl font-bold mb-4">Start Assessing Talent with Precision</h2>
          <p className="text-xl opacity-90 mb-8">
            Choose from 100+ templates or design your own assessment in minutes. Start making data-driven hiring decisions.
          </p>

          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Link href="https://portal.disamina.ai/signin" rel="noreferrer noopener" target="_blank">
              <Button size="lg" className="bg-white text-green-600 hover:bg-gray-100">
                <Download className="mr-2 h-5 w-5" />
                Free Trial
              </Button>
            </Link>
            <Button size="lg" variant="outline" className="border-white hover:bg-white bg-white/0" onClick={() => {
              if (window.Calendly) {
                window.Calendly.initPopupWidget({
                  url: 'https://calendly.com/disamina?hide_gdpr_banner=1',
                });
              } else {
                console.error('Calendly script not loaded.');
              }
              return false;
            }}>
              Request Demo Assessment
            </Button>
          </div>


        </div>
      </div>
    </section>
    <Footer />
  </div>
);

export default TalentAssessment;
